...
本分支用于保存 2026-06-19 至 2026-06-20 本地测试后已经同步到宝塔服务器的更新内容。它是从 `main` 新建的独立分支，不覆盖旧版本主分支。
...
- 清理 `data/client-cache` 和 `data/client-jobs`，避免旧 EXE 缓存继续生效。
- 重启 `toolbox-admin` 服务。

更新完成后需要在浏览器中按 `Ctrl+F5` 强制刷新后台页面；如果更新涉及客户端模板，需要重新下载并分发三个版本 EXE。
