...
function orderedButtonAreas() {
  ...
  getPositions().forEach((position) => {
    if (present.has(position.name) && !result.includes(position.name)) result.push(position.name);
  });
  state.buttons.slice().sort((a, b) =>
    Number(a.areaOrder ?? 0) - Number(b.areaOrder ?? 0) ||
    String(a.area || '').localeCompare(String(b.area || ''), 'zh-Hans-CN')
  ).forEach((button) => {
    if (button.area && !result.includes(button.area)) result.push(button.area);
  });
  return result;
}
...
function orderedButtonSections(areaFilter = '') {
  const result = [];
  state.buttons.slice().filter((button) => !areaFilter || button.area === areaFilter).sort((a, b) =>
    Number(a.areaOrder ?? 0) - Number(b.areaOrder ?? 0) ||
    Number(a.sectionOrder ?? a.sectionIndex ?? 0) - Number(b.sectionOrder ?? b.sectionIndex ?? 0) ||
    String(a.section || '').localeCompare(String(b.section || ''), 'zh-Hans-CN')
  ).forEach((button) => {
    if (button.section && !result.includes(button.section)) result.push(button.section);
  });
  return result;
}
